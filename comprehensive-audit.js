...
async function comprehensiveAudit() {
  try {
    console.log('🔍 AUDIT COMPLETO E ACCURATO DEL SISTEMA\n');
    console.log('📅 Sito online da 2+ mesi - Analisi approfondita\n');
    ...
    if (fs.existsSync(robotsPath)) {
      ...
      console.log(`📊 STATISTICHE ROBOTS.TXT:`);
      console.log(`   📄 robots.ts trovato: ✅`);
      
      // Verifica regole critiche
      const criticalRules = [
        { rule: 'sitemap:', name: 'Sitemap URL' },
        { rule: 'Disallow: /api/', name: 'API disallow' },
        { rule: 'Disallow: /_next/', name: 'Next.js disallow' },
        { rule: 'Disallow: /favicon.ico', name: 'Favicon disallow' },
        { rule: 'Disallow: /manifest.webmanifest', name: 'Manifest disallow' },
        { rule: 'Disallow: /feed.xml', name: 'Feed disallow' },
        { rule: 'Disallow: /sitemap.xml', name: 'Sitemap disallow' },
        { rule: 'Disallow: /articoli?search=*', name: 'Search disallow' }
      ];
      
      let rulesPresent = 0;
      criticalRules.forEach(({ rule, name }) => {
